...
export class DashboardComponent implements OnInit {
 dtOptions: DataTables.Settings = {};
  ...
  flag=0
  recentPatients: any;
  constructor(private get_api_physician:PhysiciansService,private patients_list:PatientsdetailsService,
    private appointment_data:SigninService,private redirection:SubscriptionService) { }

  ngOnInit() {
    let physicianId = localStorage.getItem("id")
    this.get_api_physician.getALlDoctorByMedical(physicianId).subscribe((response: any) => {
 ...
 this.appointment_data.GetAllAppointment(parentId).subscribe((result:any)=>{
